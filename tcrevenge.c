...
int main(int argc, const char *argv[]) {
  ...
  if (mode == HELP || arg_err) {
    fprintf(stderr, "Usage: %s [ {-h|--help} | {-c|--check tclinux.bin} | {-k|--kernel kernel.bin -s|--squashfs squashfs.bin -o|--output header.bin -p|--padding padding.bin [-b|--board 'board'] [-v|--version 'firmware_version']} ]\n", argv[0]);
    return arg_err;
  }
